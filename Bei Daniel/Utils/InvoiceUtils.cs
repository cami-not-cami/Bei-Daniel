...
namespace Bei_Daniel.Utils
{
    class InvoiceUtils
    {
        ...
        public class MonthlyReceiptDocument : IDocument
        {
            public string CompanyName { get; set; } = "EINZELUNT. D. IVANOV. ";
            public string CompanyAddress { get; set; } = "Wien,21. BRÜNNERSTR. 47-48";
            public string CompanyPhone { get; set; } = "+436643842241";
            ...
            public string DeliveryDate { get; set; }
            public string InvoiceNr { get; set; }
            // Dictionary: DateOnly -> List of Products for that date
            public Dictionary<DateOnly, List<Product>> ProductsByDate { get; set; } = new();
            public double Netto { get; set; }
            public double MwSt => Math.Round(Netto * 0.10, 2);
            public double Brutto => Netto + MwSt;
            public string LogoPath { get; set; } = Path.Combine(AppContext.BaseDirectory, "Helpers", "Icons", "BeiDanielLogo.png");
            ...
            void ComposeHeader(IContainer container)
            {
                container.Row(row =>
                {
                    ...
                    row.RelativeColumn(2).AlignRight().Column(col =>
                    {
                        ...
                        col.Item().PaddingTop(5).Text(CompanyName).Bold().FontSize(12);
                        col.Item().PaddingTop(2).Text(CompanyAddress).FontSize(10);
                        col.Item().PaddingTop(2).Text($"IBAN: {Iban}").FontSize(10);
                        col.Item().PaddingTop(2).Text("daniel.ivanov2707@gmail.com").FontSize(10).FontColor(Colors.Grey.Darken1);
            ...
            void ComposeBody(IContainer container)
            {
                container.PaddingTop(40).Column(col =>
                {
                    ...
                    // === Product Sections by Date ===
                    foreach (var dateGroup in ProductsByDate.OrderBy(x => x.Key))
                    {
                        // Date header
                        col.Item().PaddingTop(10).Text($"Lieferdatum: {dateGroup.Key:dd.MM.yyyy}")
                            .Bold()
                            .FontSize(12)
                            .FontColor(Colors.Grey.Darken3);

                        // Product table for this date
                        ...
                        double dateSubtotal = dateGroup.Value.Sum(p => p.Total);
                        col.Item().PaddingTop(5).AlignRight().Row(row =>
                        {
                            row.RelativeColumn(6);
                            row.RelativeColumn(2).AlignRight().Text("Zwischensumme:")
                                .Bold().FontSize(10).FontColor(Colors.Grey.Darken3);
                            row.RelativeColumn(1).AlignRight().Text($"{dateSubtotal:F2} €")
        ...
        internal static void GenerateMonthlyInvoicePdf(int restaurantId, DateOnly startDate, DateOnly endDate, Models.AppDbContext appDbContext)
        {
            ...
            // Build product list grouped by date
            var productsByDate = new Dictionary<DateOnly, List<InvoiceUtils.Product>>();
            foreach (var dateGroup in ordersByDate)
            {
                var productsForDate = new List<InvoiceUtils.Product>();
                foreach (var order in dateGroup.Value)
                {
                    var product = new InvoiceUtils.Product
                    {
                        ProductName = order.Product?.Name ?? string.Empty,
                        ProductQuantityType = order.ProductQuantityType,
                        Quantity = $"{order.Amount} {order.ProductQuantityType}",
                        UnitPrice = order.ProductPrice,
                        Total = order.ProductPrice * order.Amount
                    };
                    productsForDate.Add(product);
                }

                // Sort products within each date
                productsForDate = productsForDate.OrderBy(p => p.ProductName).ToList();
                productsByDate[dateGroup.Key] = productsForDate;
            }
            ...
            var document = new MonthlyReceiptDocument
            {
